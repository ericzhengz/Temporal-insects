...
class InsectTemporalFramework(nn.Module):
    def __init__(self, num_stages=5, feat_dim=512, num_classes=20):  # 修改默认参数
        super().__init__()
        self.num_stages = num_stages  # 5种虫态
        self.feat_dim = feat_dim
    ...
    def forward(self, visual_feat, stage_labels, class_ids=None, padding_mask=None):
        """
        适配昆虫数据集的时序特征处理
        visual_feat: [B, T, D] - B为批次大小，T为时序长度(最多5，对应5种虫态)，D为特征维度
        stage_labels: [B, T] - 每个样本序列中各时间步的虫态标签(0-4)
        class_ids: [B] - 昆虫种类标签(0-19)
        padding_mask: [B, T] - 掩码标识(有些昆虫可能不足5个虫态)
        """
    ...
    def compute_vae_loss(self, features, stage_labels, padding_mask):
        ...
        self.kl_weight = nn.Parameter(torch.tensor(0.5))  # 可学习参数 
            ...
            for i, (_, inputs, targets, stages) in enumerate(train_loader):  # 新增stages参数
                ...
                if inputs.dim() == 5:  # [B, T, C, H, W] 时序模式
                    ...
                    enhanced_feat = self.proto_pool(
                        temporal_out['temporal_feat'],  # [B*T, D]
                        targets.repeat_interleave(T),   # [B*T] 
                        stages.flatten()                # [B*T]
                    )
                    
                    logits = self._network.fc(enhanced_feat)  # [B*T, C]
