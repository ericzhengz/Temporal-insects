...
#coda_prompt
# 面向任务的动态提示输入，使用正交化约束对提示进行初始化和优化（减少任务间干扰）
class CodaPrompt(nn.Module):
    ...
    def forward(self, x_querry, l, x_block, train=False):
        ...
        if l in self.e_layers:
            e_valid = True
            B, C = x_querry.shape  # B批量 C特征维度

            K = getattr(self,f'e_k_{l}')
            A = getattr(self,f'e_a_{l}')
            p = getattr(self,f'e_p_{l}')
            # 下方，计算分区索引，用于区分任务提示词的范围
            pt = int(self.e_pool_size / (self.n_tasks))  # 每个任务的提示分配大小
            s = int(self.task_count * pt)  # 当前任务的开始索引
            f = int((self.task_count + 1) * pt)  # 结束索引
            
            # freeze/control past  冻结或控制先前任务的prompt
            if train: # 冻结先前任务提示权重，只对当前任务更新
                if self.task_count > 0:
                    ...
                    A = A[s:f]
                    p = p[s:f]
            else:  # 直接选择当前任务及其以前的所有提示（非训练）
                K = K[0:f]
                A = A[0:f]
                p = p[0:f]

            # with attention and cosine sim 注意力机制的余弦相似度计算
            # (b x 1 x d) * soft([1 x k x d]) = (b x k x d) -> attention = k x d
            a_querry = torch.einsum('bd,kd->bkd', x_querry, A)
            # # (b x k x d) - [1 x k x d] = (b x k) -> key = k x d
            n_K = nn.functional.normalize(K, dim=1)
            q = nn.functional.normalize(a_querry, dim=2)
            aq_k = torch.einsum('bkd,kd->bk', q, n_K)  # 计算每个q、k的余弦相似度
            # (b x 1 x k x 1) * [1 x plen x k x d] = (b x plen x d) -> prompt = plen x k x d
            P_ = torch.einsum('bk,kld->bld', aq_k, p)  # aq_k对p加权，得到P_，(B,L,D)
